...
class TestInstaller(DAObject):
  ...
  def push_files( self ):
    ...
    # TODO: Explore why this creates multiple pushes while the code lower down does not
    log( 'start', 'console' )
    env_example = self.env_example.content
    log( 'a', 'console' )
    example_test = self.example_test.content
    log( 'b', 'console' )
    gitignore = self.gitignore.content
    log( 'c', 'console' )
    package_json = self.package_json.content
    log( 'd', 'console' )
    run_interview_tests = self.run_interview_tests.content
    log( 'e', 'console' )
    self.repo.create_file('.env_example', 'Add .env_example', self.env_example.content, branch=self.branch_name)  # 1
    log( 'f', 'console' )
    self.repo.create_file('tests/features/example_test.feature', 'Add tests/features/example_test.feature', self.example_test.content, branch=self.branch_name)  # 2
    log( 'g', 'console' )
    self.repo.create_file('.gitignore', 'Add .gitignore', self.gitignore.content, branch=self.branch_name)  # 3
    log( 'h', 'console' )
    self.repo.create_file('package.json', 'Add package.json', self.package_json.content, branch=self.branch_name)  # 4
    log( 'i', 'console' )
    self.repo.create_file('.github/workflow/run_form_tests.yml', 'Add r.github/workflow/run_form_tests.yml', self.run_interview_tests.content, branch=self.branch_name)  # 5
    log( 'j', 'console' )
